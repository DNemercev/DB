...
class Software extends Model
{
    ...
    {
        $res = DB::table('slots')->whereBetween('slots', [20, 100])->select('*')->get();
        return $res;
    }
    ...
    /*
     * SELECT COUNT(*) AS res, softwares.title FROM slots
     * JOIN softwares
    ...
    */
    public function AllSoftwaresWhereslotsMore100() // 3.2)
    {
        $res = DB::table('slots')
            ...
            ->join('softwares', 'software_id', '=', 'slots.software_id')
            ->groupBy('software_id')
            ->having('res', '>', 100)
            ->get();
